...
if (isset($_POST['submitRatingStar'])) {
    ...
    $res=mysqli_query($conn, "SELECT email_user FROM curso_js WHERE email_user='$correo'");
    $cantidad=mysqli_num_rows($res);
    if ($cantidad==1) {
        header("location: dashboard.php");
    }else{        
        $res=mysqli_query($conn, "INSERT INTO curso_js (star_rate, email_user) VALUES ('$calif','$correo')");
        header("location: dashboard.php");
    }
}
?>
...
<head>
    ...
    <title class="challege">JS-Cuest</title>
</head>
<script>
...
<body>
    ...
    <main>
        <h3 class="title">¿JS es parecido a: ?</h3>
        <form action="#" class="form">
            <nav>
                <table class="cuest">
                    <button id="a" ><tr onclick="Eva(true);">
                        <td class="opcion" >A</td>
                        <td class="answer" id="ansa">     Lenguaje Java   </td>
                    </tr>
                    <tr onclick="Eva(false);">
                        <td class="opcion">B</td>
                        <td class="answer" id="ansb">    Lenguaje C   </td>
                    </tr>
                    <tr onclick="Eva(false);">
    ...
    <footer class="footer">
            <div class="row">
                <div id="content" class="col-lg-12">
                    <form action="JSCuest.php" method="post">
