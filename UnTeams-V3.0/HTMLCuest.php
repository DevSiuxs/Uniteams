...
if (isset($_POST['submitRatingStar'])) {
    ...
    $res=mysqli_query($conn, "SELECT email_user FROM curso_html WHERE email_user='$correo'");
    $cantidad=mysqli_num_rows($res);
    if ($cantidad==1) {
        header("location: dashboard.php");
    }else{        
        $res=mysqli_query($conn, "INSERT INTO curso_html (star_rate, email_user) VALUES ('$calif','$correo')");
        header("location: dashboard.php");
    }
}
?>
...
<head>
    ...
    <title class="challege">HTML-Cuest</title>
</head>
<script>
...
<body>
    ...
    <main>
        <h3 class="title">¿Qué es HTML?</h3>
        <form action="#" class="form">
            <nav>
                <table class="cuest">
                    <button id="a" ><tr onclick="Eva(false);">
                        <td class="opcion" >A</td>
                        <td class="answer" id="ansa">Un Lenguaje de programación</td>
                    </tr>
                    <tr onclick="Eva(true);">
                        <td class="opcion">B</td>
                        <td class="answer" id="ansb">Un lenguaje de etiquetado</td>
                    </tr>
                    <tr onclick="Eva(false);">
                        <td class="opcion">C</td>
                        <td class="answer" id="ansc">El esqueleto de una pagina web</td>
                    </tr>
                </table>
            </nav>
        </form> 
    ...
    <footer class="footer">
            <div class="row">
                <div id="content" class="col-lg-12">
                    <form action="HTMLCuest.php" method="post">
