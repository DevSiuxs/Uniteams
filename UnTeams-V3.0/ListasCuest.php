...
if (isset($_POST['submitRatingStar'])) {
    ...
    $res=mysqli_query($conn, "SELECT email_user FROM curso_listas WHERE email_user='$correo'");
    $cantidad=mysqli_num_rows($res);
    if ($cantidad==1) {
        header("location: dashboard.php");
    }else{        
        $res=mysqli_query($conn, "INSERT INTO curso_listas (star_rate, email_user) VALUES ('$calif','$correo')");
        header("location: dashboard.php");
    }
}
?>
...
<meta charset="UTF-8">
    ...
    <title class="challege">Listas-Challenge</title>
</head>
<script>
...
<header>
    ...
    <main>
        <h3 class="title">¿Cuáles son las funciones principales de una Lista?</h3>
        <form action="#" class="form">
            <nav>
                <table class="cuest">
                    <button id="a" ><tr onclick="Eva(true);">
                        <td class="opcion" >A</td>
                        <td class="answer" id="ansa">       Tipo Push          </td>
                    </tr>
                    <tr onclick="Eva(false);">
                        <td class="opcion">B</td>
                        <td class="answer" id="ansb">      Tipo Dobles       </td>
                    </tr>
                    <tr onclick="Eva(false);">
                        <td class="opcion">C</td>
                        <td class="answer" id="ansc">      Tipo Simples     </td>
                    </tr>
                </table>
            </nav>
        </form> 
    ...
    <footer class="footer">
            <div class="row">
                <div id="content" class="col-lg-12">
                    <form action="ListasCuest.php" method="post">
