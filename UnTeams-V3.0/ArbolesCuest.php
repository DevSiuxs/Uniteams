...
if (isset($_POST['submitRatingStar'])) {
    ...
    $res=mysqli_query($conn, "SELECT email_user FROM curso_arboles WHERE email_user='$correo'");
    $cantidad=mysqli_num_rows($res);
    if ($cantidad==1) {
        header("location: dashboard.php");
    }else{        
        $res=mysqli_query($conn, "INSERT INTO curso_arboles (star_rate, email_user) VALUES ('$calif','$correo')");
        header("location: dashboard.php");
    }
}
?>
...
<head>
    ...
    <title class="challege">Árboles-Cuest</title>
</head>
<script>
...
<body>
    ...
    <main>
        <h3 class="title">¿Cuáles son los tipos de recorrido hay en la estructura de datos arboles?
        </h3>
            ...
            <nav>
                <table class="cuest">
                    <button id="a" ><tr onclick="Eva(true);">
                        ...
                        <td class="answer" id="ansa">Pre-orden, Post-orden, en-orden</td>
                    </tr>
                    <tr onclick="Eva(false);">
                        <td class="opcion">B</td>
                        <td class="answer" id="ansb">Pre-orden, Post-orden, post-post-orden</td>
                    </tr>
                    <tr onclick="Eva(false);">
                        <td class="opcion">C</td>
                        <td class="answer" id="ansc">Ninguna de las anteriores</td>
                    </tr>
                </table>
            </nav>
        </form> 
    ...
    <footer class="footer">
            <div class="row">
                <div id="content" class="col-lg-12">
                    <form action="ArbolesCuest.php" method="post">
