...
if (isset($_POST['submitRatingStar'])) {
    ...
    $res=mysqli_query($conn, "SELECT email_user FROM curso_pilas WHERE email_user='$correo'");
    $cantidad=mysqli_num_rows($res);
    if ($cantidad==1) {
        header("location: dashboard.php");
    }else{        
        $res=mysqli_query($conn, "INSERT INTO curso_pilas (star_rate, email_user) VALUES ('$calif','$correo')");
        header("location: dashboard.php");
    }
}
?>
...
<meta charset="UTF-8">
    ...
    <title class="challege">Pilas-Callenge</title>
</head>
<script>
...
<body>
    ...
    <main>
        <h3 class="title">¿Cuáles son las funciones principales de una Pila?</h3>
        <form action="#" class="form">
            <nav>
                <table class="cuest">
                    <button id="a" ><tr onclick="Eva(true);">
                        <td class="opcion" >A</td>
                        <td class="answer" id="ansa">push, top, pop</td>
                    </tr>
                    <tr onclick="Eva(false);">
                        <td class="opcion">B</td>
                        <td class="answer" id="ansb">pop, push, FYLO</td>
                    </tr>
                    <tr onclick="Eva(false);">
                        <td class="opcion">C</td>
                        <td class="answer" id="ansc">FYLO, top, pop, push</td>
                    </tr>
                </table>
            </nav>
        </form> 
    ...
    <footer class="footer">
            <div class="row">
                <div id="content" class="col-lg-12">
                    <form action="PilasCuest.php" method="post">
