...
if (isset($_POST['submitRatingStar'])) {
    ...
    $res=mysqli_query($conn, "SELECT email_user FROM curso_grafos WHERE email_user='$correo'");
    $cantidad=mysqli_num_rows($res);
    if ($cantidad==1) {
        header("location: dashboard.php");
    }else{        
        $res=mysqli_query($conn, "INSERT INTO curso_grafos (star_rate, email_user) VALUES ('$calif','$correo')");
        header("location: dashboard.php");
    }
}
?>
...
<head>
    ...
    <title class="challege">Grafos-Cuest</title>
</head>
<script>
...
<body>
    ...
    <main>
        <h3 class="title">¿Cuáles estructuras de Grafos son mas comunes?</h3>
        <form action="#" class="form">
            <nav>
                <table class="cuest">
                    <button id="a" ><tr onclick="Eva(true);" value="35">
                        <td class="opcion" >A</td>
                        <td class="answer" id="ansa">Conexo y No Conexo</td>
                    </tr>
                    <tr onclick="Eva(false);">
                        <td class="opcion">B</td>
                        <td class="answer" id="ansb">Conexo y Pre-orden</td>
                    </tr>
                    <tr onclick="Eva(false);">
                        <td class="opcion">C</td>
                        <td class="answer" id="ansc">Anchura PotsOrden</td>
                    </tr>
                </table>
            </nav>
        </form> 
    ...
    <footer class="footer">
            <div class="row">
                <div id="content" class="col-lg-12">
                    <form action="GrafosCuest.php" method="post">
